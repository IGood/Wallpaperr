namespace Wallpaperr2
{
    ...
    static class WallpaperComposer
    {
        ...
        public static int MakePicture(string[] fileName, Properties.Settings settings, BackgroundWorker worker)
        {
            ...
            string outFileExt = WindowsVersionSupportsJpg ? "jpg" : "bmp";
            string outFileName = Path.Combine(Helpers.AppDataPath, $@"created.{outFileExt}");
            ...
            try
            {
                ...
                if (WindowsVersionSupportsJpg)
                {
                    ...
                    ImageCodecInfo encoder = codecs.FirstOrDefault(codec => codec.FormatID.Equals(ImageFormat.Jpeg.Guid));
                    if (encoder != null)
                    {
                        using (var encoderParams = new EncoderParameters()) // has 1 slot by default
                        {
                            encoderParams.Param[0] = new EncoderParameter(Encoder.Quality, 100L);
                            finalImg.Save(outFileName, encoder, encoderParams);
                        }
                    }
                    else
                    {
...
We'll try again later.");
        ...
        private class ComposerData
        {
            ...
            {
                this.BorderSpace = (float)settings.ForegroundMargin / 100f;
                /*
                this.MaxScale = settings.UseMaxScale ? (float)settings.MaxScale / 100f : default(float?);
                /*/
                this.MaxScale = 1f;
                //*/
                this.Thickness = (int)settings.BorderThickness;
